...
def test_creating_project(
    logged_app: Application, remove_test_projects, company, project_name, project_type
):
    app = logged_app

    # arrange
    (app.projects_page.is_loaded().open_company_projects(company).is_project_list_loaded())

    projects_num_before = (
        app.projects_page.is_loaded().is_project_list_loaded().get_project_list_locator()
    ).count()

    app.projects_page.click_create_project()

    # act
    (app.create_project_page.is_loaded().create_project(project_type, project_name))
...
def test_deleting_project(logged_app: Application, create_test_project):
    app = logged_app
    (app.projects_page.is_loaded().open_company_projects(DEFAULT_COMPANY))
...
def test_project_search(
    logged_app: Application, create_multiple_projects, search_value, expected_project_name
):
    app = logged_app
    (app.projects_page.is_loaded().is_project_list_loaded().search_project(search_value))
